...
elif selected_nav == "Roboto ML Predictions" and ENHANCED_FEATURES:
    if not st.session_state.logged_in:
                ...
                if stock_data.empty:
                    ...
                    if not is_valid:
                        ...
                        if price_change > 5:
                            ...
                            recommendations.append("🟢 Hold position with tight stop-loss")
                        elif price_change > 0:
                            recommendations.append("🟡 Monitor closely for breakout confirmation")
                        elif price_change > -2:
                            recommendations.append("🟡 Watch for support levels")
                        else:
                            recommendations.append("🔴 Consider reducing position size")

                        # Confidence-based recommendation
                        if confidence > 0.7:
                            recommendations.append("🟢 High-confidence signal, consider higher allocation")
                        elif confidence > 0.5:
                            recommendations.append("🟡 Moderate confidence, maintain regular monitoring")
                        else:
                            recommendations.append("🔴 Low confidence, avoid significant positions")

                        # Risk-based recommendation
                        if risk_score < 40:
                            recommendations.append("🟢 Favorable risk-reward profile")
                        elif risk_score < 70:
                            recommendations.append("🟡 Balanced risk profile")
                        else:
                            recommendations.append("🔴 High-risk scenario, cautious approach advised")

                        # Time horizon recommendation
                        if prediction_period == "1 Week":
                            recommendations.append("⚡ Short-term opportunity")
                        elif prediction_period == "2 Weeks":
                            recommendations.append("📈 Medium-term potential")
                        else:
                            recommendations.append("📈 Long-term investment opportunity")

                        # Strategy suggestion
                        if price_change > 0 and confidence > 0.7 and risk_score < 60:
                            recommendations.append("📈 Consider initiating position with trailing stop")
                        elif price_change > 0:
                            recommendations.append("📈 Watch for confirmation signals before entering")
                        elif price_change < 0:
                            recommendations.append("📉 Consider hedging or alternative opportunities")
                        
                        # Add performance suggestions
                        if analysis_depth != "Basic":
                            recommendations.append("📈 Consider scaling in/out based on volatility patterns")
                            recommendations.append("📉 Use options strategy to hedge downside risk")
                            recommendations.append("📊 Combine with other assets for diversification")
